...
public class MemberDeleteInfoListView extends VerticalLayout implements ClickListener {
	private static final Log log = Logs.get();
	
	// ---------表格需要的
		// 定义一个表格使用的容器对象
		private BeanItemContainer<MemberDeleteInfoBean> container = new BeanItemContainer<MemberDeleteInfoBean>(MemberDeleteInfoBean.class);
		// 每列宽度
		private static final int[] COLUMN_WIDTHS = { 120, 60, 50, 90, 180, 120, 80,80, 120 };
		// 间隙
		...
		TextField memberCard_balance = null; // 会员卡余额 余额、积分 实体卡从 crds 表获取，根据 关联 id 一一获取
 ...
 *  创建过滤器table
 * **/
	private Table createTable(BeanItemContainer<MemberDeleteInfoBean> mdInfoBeanList){
  ...
  * **/   
    private FormLayout createAddForm(final Item itemBean,final Window window){
    	...
    	okButton.addClickListener(new ClickListener() {
			...
			@Override
			public void buttonClick(ClickEvent event) {
			   mdInfoManager.removeMemberDeleteInfoBean( (String)itemBean.getItemProperty("realname").getValue(),(String)itemBean.getItemProperty("loginname").getValue());
 ...
 * **/	
	...
	private void fillContainer(Container container) {
		List<MemberDeleteInfoBean> list = mdInfoManager.getMemberDeleteInfoBeanList();
		for (Iterator iterator = list.iterator(); iterator.hasNext();) {
			MemberDeleteInfoBean mdInfo = (MemberDeleteInfoBean) iterator.next();
